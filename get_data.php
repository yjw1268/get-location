...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if(isset($_GET["gid"])){
        if(isset($_GET["num"])&&is_numeric($num)){
            $sql="SELECT gid,data_position,data_accuracy,`time` FROM alldata WHERE `gid`='$gid' ORDER BY ID DESC LIMIT $num";
        }elseif(!isset($_GET["num"])){
            $sql = "SELECT gid,data_position,data_accuracy,`time` FROM alldata WHERE `gid`='$gid' ORDER BY ID DESC";
        }
    }else{
        if (isset($_GET["num"]) && is_numeric($num)) {
            $sql = "SELECT gid,data_position,data_accuracy,`time` FROM alldata ORDER BY ID DESC LIMIT $num";
        } elseif (!isset($_GET["num"])) {
